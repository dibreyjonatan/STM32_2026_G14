...
#define COPY_BEG14_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
#define COPY_MYBUTTON_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x02,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomMybuttonHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MYBUTTON_NOTIFY_DISABLED_EVT;
                Custom_STM_App_Notification(&Notification);
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_MYBUTTON_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          }  /* if (attribute_modified->Attr_Handle == (CustomContext.CustomMybuttonHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for BEG14_SERVICE +
   *                                2 for myCharWrite +
   *                                2 for myCharNotify +
   *                                2 for MyButton +
   *                                1 for myCharNotify configuration descriptor +
   *                                1 for MyButton configuration descriptor +
   *                              = 9
   *
  ...
  COPY_BEG14_SERVICE_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomBeg14Hdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: BEG14, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: BEG14 \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomBeg14Hdle,
                          UUID_TYPE_128, &uuid,
                          SizeMycharwrite,
                          CHAR_PROP_WRITE,
   ...
   *  MyButton
   */
  COPY_MYBUTTON_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_char(CustomContext.CustomBeg14Hdle,
                          UUID_TYPE_128, &uuid,
                          SizeMybutton,
                          CHAR_PROP_NOTIFY,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_CONSTANT,
                          &(CustomContext.CustomMybuttonHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : MYBUTTON, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : MYBUTTON \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MYCHARWRITE:
      ret = aci_gatt_update_char_value(CustomContext.CustomBeg14Hdle,
                                       CustomContext.CustomMycharwriteHdle,
                                       0, /* charValOffset */
    ...
    case CUSTOM_STM_MYBUTTON:
      ret = aci_gatt_update_char_value(CustomContext.CustomBeg14Hdle,
                                       CustomContext.CustomMybuttonHdle,
                                       0, /* charValOffset */
                                       SizeMybutton, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value MYBUTTON command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value MYBUTTON command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_MYCHARWRITE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomBeg14Hdle, CustomContext.CustomMycharwriteHdle, SizeMycharwrite, pPayload);
    ...
    case CUSTOM_STM_MYCHARNOTIFY:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomBeg14Hdle, CustomContext.CustomMycharnotifyHdle, SizeMycharnotify, pPayload);
    ...
    case CUSTOM_STM_MYBUTTON:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomBeg14Hdle, CustomContext.CustomMybuttonHdle, SizeMybutton, pPayload);
